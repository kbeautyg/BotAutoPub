...
# Добавьте эти функции в начало файла scheduled_posts.py после импортов
...
def safe_markdown_format(text, parse_mode):
...
# Замените функцию send_post_preview в scheduled_posts.py на эту версию:
...
async def send_post_preview(message: Message, data: dict):
    ...
    if parse_mode and parse_mode.lower() == "markdown":
        safe_text = safe_markdown_format(text, parse_mode)
    else:
        safe_text = text
    ...
    # Отправка превью с обработкой ошибок парсинга
    try:
        ...
        if "can't parse entities" in error_msg or "can't find end of the entity" in error_msg:
            try:
                ...
                # Предупреждаем о проблеме с форматированием
                await message.answer(
                    "⚠️ **Предупреждение:** В тексте обнаружены символы, несовместимые с Markdown форматированием.\n"
                    "Пост отправлен без форматирования. Рекомендуется использовать HTML формат или экранировать специальные символы.",
                    parse_mode="Markdown"
                )
            except Exception as final_error:
                await message.answer(
                    f"❌ **Критическая ошибка предпросмотра**\n\n"
                    f"Ошибка: {str(final_error)}\n\n"
                    f"Проверьте содержимое поста и попробуйте другой формат."
                )
        else:
            await message.answer(
...
# Добавляем обработчик для предложения редактирования в scheduled_posts.py
@router.callback_query(F.data.startswith("edit_offer_"))
async def handle_edit_offer_response(callback: CallbackQuery, state: FSMContext):
    """Обработка ответа на предложение редактирования"""
    action = callback.data.split("_")[-1]  # accept или decline
    
    # Извлекаем post_id из callback_data предыдущего сообщения
    try:
        # Ищем post_id в тексте сообщения
    ...
    if action == "accept":
        # Пользователь хочет редактировать пост
        ...
        # Пользователь не хочет редактировать
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        await callback.message.edit_text(
            f"✅ **Отлично!**\n\n"
            f"Пост #{post_id} готов к публикации в запланированное время.",
            parse_mode="Markdown",
            reply_markup=keyboard
        )
        await callback.answer()
